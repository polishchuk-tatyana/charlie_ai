...
This repo implements the **text-only core** (per the task brief): the service accepts what the child typed, drives the lesson flow, and generates Charlie's response through an LLM. Voice I/O (STT/TTS) is out of scope here but can be plugged in later (see [Future work](#future-work)).
